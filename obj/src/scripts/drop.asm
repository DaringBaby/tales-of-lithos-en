...
;src/scripts/drop.c:38: drops[i].y = 0;
	...
	ld	(de), a
	ld	(hl), #0x00
;src/scripts/drop.c:39: move_sprite(33+i, 0, 0);
	ld	a, c
	add	a, #0x21
	ld	e, a
;c:\users\utente\desktop\tirocinio\gbdk-win64\gbdk\include\gb\gb.h:1973: OAM_item_t * itm = &shadow_OAM[nb];
...
;src/scripts/drop.c:35: for (int i=0; i<2; i++) {
	inc	bc
;src/scripts/drop.c:41: }
	jr	00104$
;src/scripts/drop.c:43: void check_drops(uint8_t x, uint8_t y) {
;	---------------------------------
...
_check_drops::
	...
	ld	(hl-), a
	ld	(hl), e
;src/scripts/drop.c:44: for (int i=0; i<2; i++) {
	ld	bc, #0x0000
00116$:
	ld	a, c
	sub	a, #0x02
	jp	NC, 00118$
;src/scripts/drop.c:45: if (x==drops[i].x && y==drops[i].y) {
	ld	l, c
	ld	h, b
	add	hl, hl
	add	hl, bc
	...
	sub	a, (hl)
	jr	NZ, 00117$
;src/scripts/drop.c:46: if (drops[i].drop_type == 2) {
	ldhl	sp,#4
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	cp	a, #0x02
	jr	NZ, 00108$
;src/scripts/drop.c:47: if (num_arrows < max_num_arrows) {
	ld	a, (#_num_arrows)
	ld	hl, #_max_num_arrows
	sub	a, (hl)
	jr	NC, 00109$
;src/scripts/drop.c:48: num_arrows++;
	ld	hl, #_num_arrows
	inc	(hl)
;src/scripts/drop.c:49: heal_sfx();
	push	bc
	ld	e, #b_heal_sfx
	ld	hl, #_heal_sfx
	...
	jr	00109$
00108$:
;src/scripts/drop.c:52: else if (drops[i].drop_type == 1) {
	dec	a
	jr	NZ, 00109$
;src/scripts/drop.c:53: current_hp += 5;
	ld	a, (_current_hp)
	add	a, #0x05
	ld	(#_current_hp),a
;src/scripts/drop.c:54: if (current_hp > max_hp) {
	ld	a, (#_max_hp)
	ld	hl, #_current_hp
	sub	a, (hl)
	jr	NC, 00109$
;src/scripts/drop.c:55: current_hp = max_hp;
	ld	a, (#_max_hp)
	ld	(#_current_hp),a
;src/scripts/drop.c:56: heal_sfx();
	push	bc
	ld	e, #b_heal_sfx
	ld	hl, #_heal_sfx
	call	___sdcc_bcall_ehl
	pop	bc
00109$:
;src/scripts/drop.c:59: drops[i].x = 0;
	pop	hl
	ld	(hl), #0x00
	push	hl
;src/scripts/drop.c:60: drops[i].y = 0;
	ldhl	sp,	#2
	ld	a, (hl+)
	ld	h, (hl)
	ld	l, a
	ld	(hl), #0x00
;src/scripts/drop.c:61: drops[i].drop_type = 0;
	ldhl	sp,	#4
	ld	a, (hl+)
	ld	h, (hl)
	ld	l, a
	ld	(hl), #0x00
;src/scripts/drop.c:62: move_sprite(33+i, 0, 0);
	ld	a, c
	add	a, #0x21
	ld	e, a
;c:\users\utente\desktop\tirocinio\gbdk-win64\gbdk\include\gb\gb.h:1973: OAM_item_t * itm = &shadow_OAM[nb];
